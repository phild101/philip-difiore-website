...
export default function ArtCollection(){
 ...
 {room<0?<><header className="art-index-top"><span>Philip Di Fiore / Collection II</span><Link href="/catalog">All 19 studies ↗</Link></header><div className="art-index-intro"><h1>Six works<br/><em>for a browser.</em></h1><p>Images, intervals, traces.<br/>Each study is a different way<br/>of being with the films.</p></div><div className="art-index-grid">{studies.map((s,i)=>{const C=works[i];return <article className="art-study" key={s.title}><div className="art-preview"><div className="art-preview-screen" aria-hidden="true" inert><C watch={noop} preview/></div></div><button className="art-study-link" onClick={()=>enter(i)} aria-label={'Enter '+s.title}><span className="art-study-no">0{i+1}</span><span className="art-study-name">{s.title}</span><span className="art-study-arrow">↗</span></button><p className="art-study-rule">{s.material}</p></article>;})}</div><footer className="art-index-bottom"><span>Six interactive studies using Philip’s existing films and imagery.</span><Link href="/collection-one/">Collection I is preserved here ↗</Link></footer></>:<><Active key={room} watch={setScreening} paused={paused}/>{chrome?<nav className="art-review-dock" aria-label="Design study controls"><span className="art-dock-title">II / 0{room+1}<span className="dock-title-name"> · {work.title}</span></span><Link href="/catalog">All 19</Link><button onClick={()=>enter(-1)}>II</button><button onClick={()=>setNotes(true)}>Notes</button><button onClick={()=>setPaused(p=>!p)} aria-pressed={paused} title="Hold page animations; film playback is separate">{paused?'Animate':'Hold'}</button><button onClick={()=>enter((room+1)%6)} aria-label="Next study">→</button><button className="art-hide" onClick={()=>setChrome(false)} aria-label="Hide study controls">−</button></nav>:<button className="art-show-controls" onClick={()=>setChrome(true)} aria-label="Show study controls">+</button>}</>}
