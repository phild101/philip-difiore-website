...
One audio element lives in the root layout, so playback and its current party queue survive navigation between Film, Music and Info. A minimal persistent strip appears after selecting a recording, only on pages other than the Recording Parties listening page. The listening page uses its in-page transport; hiding the strip and its spacer there does not pause or reset the shared audio. The strip returns when navigating elsewhere, including the Music cover. The current label reads “Recording Party 2/ Tape 1” and links back to that playing party. It includes elapsed/total time, a seek slider, simple solid previous/play-pause/next icons, and an X that stops and clears playback. Restart has been removed. Previous/next remain within the playing party, disabled at queue boundaries; explicit skips start the chosen tape. The detailed page slider remains synchronized beneath the selected tape. Opening a film stops music and dismisses the strip; closing the film does not bring it back. Full browser reloads or leaving the site also end playback.

The strip uses the current Film/Music cover's background and header ink instead of a jarring white background; Info and the spare archive keep white/charcoal. No artwork, shadows, rollover scale or decorative controls. Desktop height is 48px; mobile uses two compact rows with measured bottom clearance. The scrubber previews a drag locally, then commits the seek; keyboard input works and the thumb has a screen-reader label/time value. Metadata readiness controls scrubbing without blocking later buffering. The original text-control version is preserved in Site version 74 / commit `7ccfc2c1b38606a6237c7c8731f6fc486d37b546`.
