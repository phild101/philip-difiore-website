...
export function RoundTwoStudy({ direction }: { direction: string }) {
  return <div className="lr-study"><nav className="lr-switcher" aria-label="Second set of landing page studies"><a href="/landing-studies/all">All studies</a><div>{openings.map((item, i) => <a key={item.id} href={`${base}/${item.id}`} aria-label={`Preview ${item.name}`} aria-current={direction === item.id ? 'page' : undefined}><span>{String(i + 6).padStart(2, '0')}</span><span className="lr-switch-name">{item.name}</span></a>)}</div></nav><Surface direction={direction} /></div>;
}
