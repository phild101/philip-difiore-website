...
One audio element lives in the root layout, so playback and its current party queue survive navigation between Film, Music and Info. A minimal persistent strip appears only after selecting a recording. The current label reads “Now playing: Recording Party 2/ Tape 1” and links back to that playing party. It includes elapsed/total time, a seek slider, simple solid previous/play-pause/next icons, and an X that stops and clears playback. Restart has been removed. Previous/next remain within the playing party, disabled at queue boundaries; explicit skips start the chosen tape. The detailed page slider remains synchronized beneath the selected tape. Opening a film stops music and dismisses the strip; closing the film does not bring it back. Full browser reloads or leaving the site also end playback.
