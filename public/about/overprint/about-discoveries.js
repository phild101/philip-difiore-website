...
(() => {
  const host=window.parent!==window?window.parent.location:new URL('/reseda-studies/overprint-centered',window.location.origin);
  const base=host.origin+host.pathname+host.search+'#featured/';
  document.querySelector('[data-movable=home]').href=base+'if-you-call';
  const selected=new URLSearchParams(window.location.search).get('featured')||'if-you-call';
  document.querySelector('[data-movable=featured]').href=base+encodeURIComponent(selected);
  const notes={},topics={};
