...
const dialog=document.querySelector('#trail-dialog');let path=[{id:'bio',via:'',scroll:0}],busy=false,activeScene=null,flightSerial=0,animations=[];
const reduced=matchMedia('(prefers-reduced-motion: reduce)');
const esc=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const word=(label,id)=>`<button class="word" data-go="${esc(id)}">${esc(label)}</button>`;
...
if(id==='bio'){el.innerHTML=`<div class="scene-content entry"><h1 class="signature" tabindex="-1"><span class="given">PHILIP</span><span class="family">DI FIORE</span><span class="dots">…</span></h1><article class="bio" aria-label="Biography">${bio.map(p=>'<p>'+prose(p)+'</p>').join('')}</article></div>`;return el}
...
function travelFor(source){const room=stage.getBoundingClientRect(),r=source?.getBoundingClientRect();const step=path.length;
return {x:r?(r.left+r.width/2-room.left)/room.width:.5,y:r?(r.top+r.height/2-room.top)/room.height:.5,dx:[.88,.52,-.76,.64][step%4],dy:[-.2,.7,.34,-.52][step%4],turn:step%2?1.2:-1.2,portal:step%3===1,label:source?.textContent||''}}
function show(direction=0,travel=null){
 ...
 const duration=t.portal?1120:940,ease='cubic-bezier(.65,0,.2,1)';
 const far=`translate(${-t.dx*w}px,${-t.dy*h}px) scale(${t.portal?2.3:1.2}) rotate(${-t.turn}deg)`;
 const near=`translate(${t.dx*w*.85}px,${t.dy*h*.85}px) scale(${t.portal?.48:.82}) rotate(${t.turn}deg)`;
 ...
 animations.push(next.animate([{transform:enter,opacity:0},{transform:enter,opacity:0,offset:.12},{transform:settled,opacity:1}],{duration,easing:ease,fill:'both'}));
 if(t.portal&&t.label){
  ...
  const pw=portal.offsetWidth,ph=portal.offsetHeight,peak=Math.max(w/Math.max(pw,1)*1.3,h/Math.max(ph,1)*1.25);
  const start=`translate(${t.x*w-pw/2}px,${t.y*h-ph/2}px) scale(.42)`;
  const mid=`translate(${w/2-pw/2}px,${h/2-ph/2}px) scale(${peak}) rotate(${-t.turn}deg)`;
  const end=`translate(${w/2-pw/2-t.dx*w*.16}px,${h/2-ph/2-t.dy*h*.16}px) scale(${peak*1.7}) rotate(${-t.turn*1.5}deg)`;
...
function go(id,source){if(busy||!notes[id])return;remember();const travel=travelFor(source);path=[...path,{id,via:source?.textContent||notes[id].title,travel,scroll:0}];history.pushState({rabbit:true,path},'', '#'+id);show(1,travel)}
...
window.addEventListener('popstate',()=>{if(history.state?.rabbit){const previous=path,nextPath=history.state.path;const direction=nextPath.length<previous.length?-1:1;const travel=direction<0?previous.at(-1).travel:nextPath.at(-1).travel;path=nextPath;show(direction,travel)}else{location.reload()}});
